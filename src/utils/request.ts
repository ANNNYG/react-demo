...
let isRefreshing: boolean = false; // 刷新Token中
const tempRequests: (() => void)[] = []; // 暂存请求

/**
 * 保存刷新token时和token失效时产生的请求
 * @param {string} url 地址
 * @param {RequestInit} options 参数
 * @param {boolean} needToken 是否需要携带token
 */
const addTemp = (url: string, options: RequestInit, needToken: boolean) =>
  new Promise(resolve => {
